...
class RewardShaper:
    ...
    def shape(obs, next_obs, action, reward):
        ...
        # Determine target and reward progress
        if pass_stat < 4:  # Passenger not picked up
            target = stations[pass_stat]
        else:  # Passenger picked up
            target = stations[dest_stat]
        
        curr_dist = get_dist((t_row, t_col), target)
        next_dist = get_dist((next_t_row, next_t_col), target)
        dist_change = next_dist - curr_dist
        new_reward -= dist_change * 2.0  # Positive when distance decreases
        
        # Reward successful pickup
        if action == 4 and pass_stat < 4 and next_pass_stat == 4:
            new_reward += 5.0
        
        # Penalize wrong pickup/dropoff
        if action == 4 and (pass_stat >= 4 or (t_row, t_col) != stations[pass_stat]):
            new_reward -= 1.0
        if action == 5 and (pass_stat != 4 or (t_row, t_col) != stations[dest_stat]):
            new_reward -= 1.0
