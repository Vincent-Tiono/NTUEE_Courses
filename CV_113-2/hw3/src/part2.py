...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            marker_corners, marker_ids, rejected_candidates  = cv2.aruco.detectMarkers(frame, arucoDict, parameters=arucoParameters)
            corner_int = np.round(marker_corners[0][0]).astype(int)

            # TODO: 2.find homograpy
            # function call to solve_homography()
            homography_matrix = solve_homography(ref_corns, corner_int)

            # TODO: 3.apply backward warp
            # function call to warping()
            warping(ref_image, frame, homography_matrix,
                    np.min(corner_int,axis=0)[1], 
                    np.max(corner_int,axis=0)[1], 
                    np.min(corner_int,axis=0)[0], 
                    np.max(corner_int,axis=0)[0], 
                    direction='b')
