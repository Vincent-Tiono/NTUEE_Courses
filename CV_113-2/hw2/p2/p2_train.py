...
def plot_learning_curve(
    ...
    training_accuracy = result_lists['train_acc']
    training_loss = result_lists['train_loss']
    validation_accuracy = result_lists['val_acc']
    validation_loss = result_lists['val_loss']

    # Ensure output directory exists
    os.makedirs(logfile_dir, exist_ok=True)

    # Generate and save training accuracy plot
    plt.figure()
    ...
    plt.grid(True)
    plt.savefig(os.path.join(logfile_dir, 'train_accuracy_curve.png'))
    plt.close()

    # Generate and save validation accuracy plot
    plt.figure()
    ...
    plt.grid(True)
    plt.savefig(os.path.join(logfile_dir, 'valid_accuracy_curve.png'))
    plt.close()
    ...
    plt.grid(True)
    plt.savefig(os.path.join(logfile_dir, 'valid_loss_curve.png'))
    plt.close()
    ...
    for epoch in range(cfg.epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch_data in enumerate(val_loader):
                # Display progress
                sys.stdout.write(f'\r[{epoch + 1}/{cfg.epochs}] Validation batch: {batch_idx + 1} / {len(val_loader)}')
                sys.stdout.flush()
                
                # Load and prepare batch data
                input_images = batch_data['images'].to(device)
                target_labels = batch_data['labels'].to(device)
                
                # Perform forward pass
                model_output = model(input_images)
                
                # Compute loss
                batch_loss = criterion(model_output, target_labels)
                
                # Calculate correct predictions
                predicted_labels = torch.argmax(model_output, dim=1)
                val_correct += torch.sum(predicted_labels == target_labels)
        ...
        if is_better:
            ...
            torch.save(model.state_dict(),
                       os.path.join(model_save_dir, f'{cfg.model_type}_model_best.pth'))
            best_acc = val_acc
...
def main():
    ...
    ##### MODEL #####
    ##### TODO: check model.py #####
    model_save_dir = os.path.join('./checkpoint', exp_name, 'mynet_0411_morning')
