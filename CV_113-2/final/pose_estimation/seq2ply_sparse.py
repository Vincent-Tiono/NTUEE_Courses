...
def process_sparse_sequence(data):
    # 完整輸入路徑：例如 7SCENES/chess/test/seq-03
    sequence_path = os.path.join(data[0], data[1])

    # 輸出檔案路徑：例如 test/chess-seq-03.ply
    scene = data[1].split("/")[0]
    seq = data[1].split("/")[-1]
    ply_path = os.path.join("bonus", f"{scene}-{seq}.ply")

    print(f"🔄 處理中: {sequence_path} → {ply_path}")
    seq2ply(sequence_path, ply_path, kf_every=1, voxel_grid_size=7.5e-3)
    print(f"✅ 完成: {ply_path}")
...
if __name__ == '__main__':
    ...
    data_list = [[dataset_root, d] for d in data_dirs]
    data_sparse_list = [[dataset_root, d] for d in data_sparse_dirs]
    ...
    # 不使用多執行緒處理
    # for data_dir in data_sparse_list:
    #     process_sparse_sequence(data_dir)
